...
def test_rod_converts_electricity_to_heat():
    ...
    data = D.make_data([0], [0.30], [0.0], [0.0], [0.0], [9.9], cop=3.0)
    m = B.build_model(data, cfg); B.solve(m); r = B.extract_results(m)
...
def test_heatpump_applies_cop():
    ...
    data = D.make_data([0], [0.30], [0.0], [0.0], [0.0], [9.0], cop=3.0)
    m = B.build_model(data, cfg); B.solve(m); r = B.extract_results(m)
...
def test_chp_couples_heat_and_power():
    ...
    data = D.make_data([0], [0.30], [0.0], [0.0], [20.0], [30.0], cop=3.0)
    m = B.build_model(data, cfg); B.solve(m); r = B.extract_results(m)
...
def test_chp_respects_minimum_load():
    ...
    cfg["rod"] = {"q_max": 100.0, "eta": 0.99}
    data = D.make_data([0], [0.30], [0.0], [0.0], [0.0], [10.0], cop=3.0)
    m = B.build_model(data, cfg); B.solve(m); r = B.extract_results(m)
...
def test_heat_storage_shifts_across_hours():
    ...
    data = D.make_data([0, 1], [0.10, 0.90], [0, 0], [0, 0], [0, 0], [0, 10], cop=3.0)
    m = B.build_model(data, cfg); B.solve(m); r = B.extract_results(m)

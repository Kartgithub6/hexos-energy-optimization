...
def test_stitched_length_matches_horizon():
    data = _data(72)
    r = R.solve_rolling(data, _full_system_cfg(), window_h=48, commit_h=24, verbose=False)
    assert len(r["t"]) == 72
...
def test_storage_state_is_continuous_across_seam():
    ...
    data = _data(48)
    r = R.solve_rolling(data, cfg, window_h=48, commit_h=24, verbose=False)
...
def test_lookahead_beats_no_lookahead_on_storage():
    ...
    data = _data(96)
    cfg = _full_system_cfg()
    no_look = R.solve_rolling(data, cfg, window_h=24, commit_h=24, verbose=False)
    look = R.solve_rolling(data, cfg, window_h=48, commit_h=24, verbose=False)

...
def test_import_equals_demand_when_no_supply():
    ...
    data = make_data([0, 1, 2], [0.30] * 3, [0.05] * 3, [0] * 3, [4, 6, 2], [0] * 3, cop=3.0)
    m = build_model(data, base_cfg()); solve(m); r = extract_results(m)
...
def test_pv_exactly_covers_demand():
    ...
    data = make_data([0, 1, 2], [0.30] * 3, [0.05] * 3, [5] * 3, [5] * 3, [0] * 3, cop=3.0)
    m = build_model(data, base_cfg()); solve(m); r = extract_results(m)
...
def test_battery_arbitrages_price_spread():
    ...
    data = make_data([0, 1], [0.10, 0.50], [0, 0], [0, 0], [2, 2], [0, 0], cop=3.0)
    m = build_model(data, with_battery(base_cfg(), 5.0, 5.0)); solve(m); r = extract_results(m)
